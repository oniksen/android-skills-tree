...
export default function LevelTabs({ levels }: { levels: Level[] }) {
  ...
  return (
    <div className="flex gap-2 flex-nowrap overflow-x-auto nav-md:flex-wrap nav-md:overflow-x-visible border-b border-slate-800 pb-4">
      {[...levels].sort((a,b) => a.levelOrder - b.levelOrder).map((level) => {
        ...
        return (
          <Link key={level.slug} href={`/tree/${level.slug}`}
            className={`shrink-0 px-4 py-2.5 rounded-lg text-base font-medium transition-all ${
